                   ...
                   <?php 
						if(isset($_GET['edit'])) { 
							...
							echo "
								...
								<div class='form-group'>
									<label> product Category</label>
									<input class='form-control' type='text' name='product_category' value =$product_category>
									<select class='form-control' style='height:100%;' name='product_category' value=$product_category>

										<option hidden>$product_category</option>  
										<option value='Paper Carft'    >Paper Carft</option>
										<option value='Metal Carft'    >Metal Carft</option>
										<option value='Beauty Product' >Beauty Product</option>
										<option value='Decorative Product'>Decorative Product</option>
									</select> 
								</div>
								<div class='form-group'>
							...
							<form class="col-md-3" method="POST" style="border-right:  1px solid black;"  enctype="multipart/form-data">
								...
								<label>Product Category<br>  </label>
									...
									<select class="form-control" style="height:100%;" name="product_category">
                        ...
                        <table class="table table-responsive table-condensed">
                            <thead>
                                <tr>
                                    ...
                                    <th id="mystyle">product_category</th>
                            ...
                            <tbody>
                            	<?php 
									$sql = "SELECT * FROM `products` WHERE product_category='Decorative Product'";
									...
									while($row = mysqli_fetch_assoc($result)){
										echo "
											<tr>
												...
												<td id='mystyle'>$row[product_category]</td>
												...
												<td id='mystyle'>
													<a href='decorative_product.php?edit=$row[product_id]'>
														<button class='btn btn-primary'>Edit</button>
													</a>
												</td>
												<td id='mystyle'>
													<a href='decorative_product.php?product_id=$row[product_id]'>
...
<?php 
	if(isset($_POST['submit'])){
		...
		$insert_query = "INSERT INTO `products`(`product_cat`, `product_category`, `product_title`, `product_price`, `product_desc`, `product_image`, `product_keywords`) VALUES ('$product_cat','$product_category', '$product_title', '$product_price','$product_desc', '$product_image', '$product_keywords')";
		
		if(mysqli_query($con, $insert_query)) { ?>
			<script>window.location ='decorative_product.php'</script>
			
			<?php	
		}	
	}
    $insert_sql = "INSERT INTO `products`(`product_id`, `product_cat`, `product_category`, `product_title`, `product_price`, `product_desc`, `product_image`, `product_keywords`) VALUES()";
	...
	if( isset($_GET['updateid'])) {
		...
		$product_category = mysqli_real_escape_string($con,$_GET['product_category']);
		...
		$update_sql = "UPDATE products SET product_cat ='$product_cat',product_category ='$product_category',product_title ='$product_title', product_price='$product_price',product_desc='$product_desc',product_image='$product_image', product_keywords='$product_keywords' WHERE product_id='$_GET[updateid]'";
