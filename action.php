...
if (isset($_POST["Common"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$sql = "SELECT a.product_id,a.product_title,a.product_price_after_discount,a.product_image_name,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id='$_SESSION[uid]'";
	}else{
		//When user is not logged in this query will execute
		$sql = "SELECT a.product_id,a.product_title,a.product_price_after_discount,a.product_image_name,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.ip_add='$ip_add' AND b.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
	if (isset($_POST["getCartItem"])) {
		...
		if (mysqli_num_rows($query) > 0) {
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				$product_price_after_discount = $row["product_price_after_discount"];
				$product_image_name = $row["product_image_name"];
				$cart_item_id = $row["id"];
				$qty = $row["qty"];
				$total_price=($total_price+($qty*$product_price_after_discount));
				echo '
					
                    
                    <div class="product-widget">
												<div class="product-img">
													<img src="product_images/'.$product_image_name.'" alt="">
												</div>
												<div class="product-body">
													<h3 class="product-name"><a href="#">'.$product_title.'</a></h3>
													<h4 class="product-price"><span class="qty">'.$n.'</span>&#8377;'.$product_price_after_discount.'</h4>
												</div>
						...
						<tr>
							<td data-th="Product" >
								<div class="row">
									...
									<div class="col-sm-4 "><img src="product_images/'.$product_image_name.'" style="height: 70px;width:75px;"/>
							...
							<td data-th="Price"><input type="text" class="form-control price" value="'.$product_price_after_discount.'" readonly="readonly"></td>
							<td data-th="Quantity">
								<input type="text" class="form-control qty" value="'.$qty.'" >
							</td>
							<td data-th="Subtotal" class="text-center"><input type="text" class="form-control total" value="'.$product_price_after_discount.'" readonly="readonly"></td>
							...
							while($row=mysqli_fetch_array($query)){
								...
								echo  	
									...
									<input type="hidden" name="item_name_'.$x.'" value="'.$row["product_title"].'">
								     ...
								     <input type="hidden" name="amount_'.$x.'" value="'.$row["product_price_after_discount"].'">
